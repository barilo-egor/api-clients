...
import tgb.cryptoexchange.apiclients.service.ClientRefreshTokenService;
import tgb.cryptoexchange.apiclients.service.ClientService;
import tgb.cryptoexchange.apiclients.service.JwtService;
...
class AuthenticationManagerServiceTest {
    ...
    void should_returnTokenPair_when_refreshTokenIsValid() {
        ...
        ClientRefreshTokenDTO dbToken = ClientRefreshTokenDTO.builder().token("valid_token").clientId(1L).expiresAt(Instant.now().plusSeconds(60)).build();
        ClientDTO client = ClientDTO.builder().id(1L).username("user").password("encoded_pass").build();
    ...
    void should_throwUnauthorizedException_when_refreshTokenIsExpired() {
        ...
        ClientRefreshTokenDTO dbToken = ClientRefreshTokenDTO.builder().token("expired_token").clientId(1L).expiresAt(Instant.now().minusSeconds(60)).build();
