...
class WithdrawalIT extends BaseIntegrationTest {
    ...
    void updateWithdrawal_Success() {
        Client client = clientRepository.save(Client.builder()
                ...
                .apiKey("key").apiKeyPreview("pre").secret("sec").registeredAt(Instant.now()).orderTimeoutSeconds(900).build());
    ...
    void updateWithdrawal_EmptyValues_ShouldUpdateAsEmpty() {
        Client client = clientRepository.save(Client.builder().username("clear_user").password("p").status(ClientStatus.ACTIVE)
                .apiKey("k").apiKeyPreview("p").secret("s").registeredAt(Instant.now()).orderTimeoutSeconds(900).build());
    ...
    void updateWithdrawal_NotFound_ThrowsException() {
        ...
        assertThat(badRequest.getFieldViolations(0).getDescription()).isEqualTo("Record not found for the provided ID.");
    }
